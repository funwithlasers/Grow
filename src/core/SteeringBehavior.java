/**
 * Desc:   class to encapsulate steering behaviors for a Sprite
 * 
...
//------------------------------------------------------------------------
    ...
    private Sprite m_pTargetAgent1;
    private Sprite m_pTargetAgent2;
    //the current target
    ...
    //the distance (squared) a sprite has to be from a path waypoint before
    ...
    //Arrive makes use of these to determine how quickly a sprite
     ...
     * sprite has left to apply and then applies that amount of the
     * force to add.
     */
    private boolean AccumulateForce(Vector2D RunningTot,
        ...
        double MagnitudeRemaining = m_pSprite.MaxForce() - MagnitudeSoFar;
        ...
        //does not exceed the maximum force available to this sprite, just
    ...
    private void CreateFeelers() {
        ...
        m_Feelers.add(add(m_pSprite.Pos(), mul(m_dWallDetectionFeelerLength, m_pSprite.Heading())));

        //feeler to left
        Vector2D temp = new Vector2D(m_pSprite.Heading());
        Vec2DRotateAroundOrigin(temp, HalfPi * 3.5f);
        m_Feelers.add(add(m_pSprite.Pos(), mul(m_dWallDetectionFeelerLength / 2.0f, temp)));

        //feeler to right
        temp = new Vector2D(m_pSprite.Heading());
...
/////////////////////////////////////////////////////////////////////////////// START OF BEHAVIORS
    ...
    private Vector2D Seek(Vector2D TargetPos) {
        Vector2D DesiredVelocity = mul(Vec2DNormalize(sub(TargetPos, m_pSprite.Pos())),
                m_pSprite.MaxSpeed());

        return sub(DesiredVelocity, m_pSprite.Velocity());
    }
 ...
 /* const double PanicDistanceSq = 100.0f * 100.0;
        if (Vec2DDistanceSq(m_pSprite.Pos(), target) > PanicDistanceSq)
        {
        return new Vector2D(0,0);
        }
         */

        Vector2D DesiredVelocity = mul(Vec2DNormalize(sub(m_pSprite.Pos(), TargetPos)),
                m_pSprite.MaxSpeed());
    ...
    private Vector2D Arrive(Vector2D TargetPos, Deceleration deceleration) {
        Vector2D ToTarget = sub(TargetPos, m_pSprite.Pos());
        ...
        if (dist > 0) {
            ...
            speed = Math.min(speed, m_pSprite.MaxSpeed());
     ...
     *  evader
     */
    private Vector2D Pursuit(final Sprite evader) {
        //if the evader is ahead and facing the agent then we can just seek
        //for the evader's current position.
        Vector2D ToEvader = sub(evader.Pos(), m_pSprite.Pos());

        double RelativeHeading = m_pSprite.Heading().Dot(evader.Heading());

        if ((ToEvader.Dot(m_pSprite.Heading()) > 0)
        ...
        double LookAheadTime = ToEvader.Length()
                / (m_pSprite.MaxSpeed() + evader.Speed());
     ...
     */
    private Vector2D Evade(final Sprite pursuer) {
        // Not necessary to include the check for facing direction this time

        Vector2D ToPursuer = sub(pursuer.Pos(), m_pSprite.Pos());
        ...
        double LookAheadTime = ToPursuer.Length()
                / (m_pSprite.MaxSpeed() + pursuer.Speed());
    ...
    private Vector2D Wander() {
        ...
        double JitterThisTimeSlice = m_dWanderJitter * m_pSprite.getTimeElapsed();
        ...
        return sub(Target, m_pSprite.Pos());
    }
    ...
    private Vector2D ObstacleAvoidance(List<BaseGameEntity> obstacles) {
        ...
        m_dDBoxLength = Prm.MinDetectionBoxLength
                + (m_pSprite.Speed() / m_pSprite.MaxSpeed())
                * Prm.MinDetectionBoxLength;

        //tag all obstacles within range of the box for processing
        m_pSprite.World().TagObstaclesWithinViewRange(m_pSprite, m_dDBoxLength);
        ...
        while (it.hasNext()) {
            ...
            if (curOb.IsTagged()) {
                ...
                if (LocalPos.x >= 0) {
                    ...
                    double ExpandedRadius = curOb.BRadius() + m_pSprite.BRadius();
    ...
    private Vector2D WallAvoidance(final List<Wall2D> walls) {
        ...
        for (int flr = 0; flr < m_Feelers.size(); ++flr) {
            ...
            for (int w = 0; w < walls.size(); ++w) {
                if (LineIntersection2D(m_pSprite.Pos(),
                        m_Feelers.get(flr),
                        walls.get(w).From(),
                        walls.get(w).To(),
                        DistToThisIPRef,
     ...
     */
    Vector2D Separation(final List<Sprite> neighbors) {
        Vector2D SteeringForce = new Vector2D();
        ...
        for (int a = 0; a < neighbors.size(); ++a) {
            ...
            if ((neighbors.get(a) != m_pSprite) && neighbors.get(a).IsTagged()
                    && (neighbors.get(a) != m_pTargetAgent1)) {
                Vector2D ToAgent = sub(m_pSprite.Pos(), neighbors.get(a).Pos());
     ...
     * of its neighbors
     */
    private Vector2D Alignment(final List<Sprite> neighbors) {
        //used to record the average heading of the neighbors
        Vector2D AverageHeading = new Vector2D();

        //used to count the number of sprites in the neighborhood
        int NeighborCount = 0;

        //iterate through all the tagged sprites and sum their heading vectors
        for (int a = 0; a < neighbors.size(); ++a) {
            //make sure *this* agent isn't included in the calculations and that
            //the agent being examined  is close enough ***also make sure it doesn't
        ...
        //if the neighborhood contained one or more sprites, average their
        //heading vectors.
        if (NeighborCount > 0) {
            AverageHeading.div((double) NeighborCount);
            AverageHeading.sub(m_pSprite.Heading());
        }
     ...
     */
    private Vector2D Cohesion(final List<Sprite> neighbors) {
     ...
     */
    private Vector2D SeparationPlus(final List<Sprite> neighbors) {
        Vector2D SteeringForce = new Vector2D();

        //iterate through the neighbors and sum up all the position vectors
        for (BaseGameEntity pV = m_pSprite.World().CellSpace().begin();
             !m_pSprite.World().CellSpace().end();
             pV = m_pSprite.World().CellSpace().next()) {
            //make sure this agent isn't included in the calculations and that
            //the agent being examined is close enough
            if (pV != m_pSprite) {
                Vector2D ToAgent = sub(m_pSprite.Pos(), pV.Pos());
     ...
     *  USES SPACIAL PARTITIONING
     */
    private Vector2D AlignmentPlus(final List<Sprite> neighbors) {
        //This will record the average heading of the neighbors
        Vector2D AverageHeading = new Vector2D();

        //This count the number of sprites in the neighborhood
        double NeighborCount = 0.0;

        //iterate through the neighbors and sum up all the position vectors
        for (MovingEntity pV = m_pSprite.World().CellSpace().begin();
             !m_pSprite.World().CellSpace().end();
             pV = m_pSprite.World().CellSpace().next()) {
            //make sure *this* agent isn't included in the calculations and that
            //the agent being examined  is close enough
     ...
     * USES SPACIAL PARTITIONING
     */
    private Vector2D CohesionPlus(final List<Sprite> neighbors) {
        //first find the center of mass of all the agents
        Vector2D CenterOfMass = new Vector2D(), SteeringForce = new Vector2D();
     ...
     * position the sprite between them
     */
    private Vector2D Interpose(final Sprite AgentA, final Sprite AgentB) {
        //first we need to figure out where the two agents are going to be at 
        //time T in the future. This is approximated by determining the time
        //taken to reach the mid way point at the current time at at max speed.
        Vector2D MidPoint = div(add(AgentA.Pos(), AgentB.Pos()), 2.0);

        double TimeToReachMidPoint = Vec2DDistance(m_pSprite.Pos(), MidPoint)
                / m_pSprite.MaxSpeed();
    ...
    private Vector2D Hide(final Sprite hunter, final List<BaseGameEntity> obstacles) {
        ...
        while (it.hasNext()) {
            ...
            //spot to the agent
            double dist = Vec2DDistanceSq(HidingSpot, m_pSprite.Pos());
    ...
    private Vector2D FollowPath() {
        ...
        if (Vec2DDistanceSq(m_pPath.CurrentWaypoint(), m_pSprite.Pos())
    ...
    /**
     * Produces a steering force that keeps a sprite at a specified offset
     * from a leader sprite
     */
    private Vector2D OffsetPursuit(final Sprite leader,
        ...
        Vector2D ToOffset = sub(WorldOffsetPos, m_pSprite.Pos());

        //the lookahead time is propotional to the distance between the leader
        //and the pursuer; and is inversely proportional to the sum of both
        //agent's velocities
        double LookAheadTime = ToOffset.Length()
                / (m_pSprite.MaxSpeed() + leader.Speed());
...
//
//------------------------------------------------------------------------
    public SteeringBehavior(Sprite agent) {
...
//---------------------------------dtor ----------------------------------
    ...
    public void RenderAids() {
        ...
        if (KEYDOWN(KeyEvent.VK_INSERT)) {
            m_pSprite.SetMaxForce(m_pSprite.MaxForce() + 1000.0f * m_pSprite.getTimeElapsed());
        }
        if (KEYDOWN(KeyEvent.VK_DELETE)) {
            if (m_pSprite.MaxForce() > 0.2f) {
                m_pSprite.SetMaxForce(m_pSprite.MaxForce() - 1000.0f * m_pSprite.getTimeElapsed());
            }
        }
        if (KEYDOWN(KeyEvent.VK_HOME)) {
            m_pSprite.SetMaxSpeed(m_pSprite.MaxSpeed() + 50.0f * m_pSprite.getTimeElapsed());
        }
        if (KEYDOWN(KeyEvent.VK_END)) {
            if (m_pSprite.MaxSpeed() > 0.2f) {
                m_pSprite.SetMaxSpeed(m_pSprite.MaxSpeed() - 50.0f * m_pSprite.getTimeElapsed());
            }
        }

        if (m_pSprite.MaxForce() < 0) {
            m_pSprite.SetMaxForce(0.0f);
        }
        if (m_pSprite.MaxSpeed() < 0) {
            m_pSprite.SetMaxSpeed(0.0f);
        }
        
        if (m_pSprite.ID() == 0) {
            gdi.TextAtPos(5, NextSlot, "MaxForce(Ins/Del):");
            gdi.TextAtPos(160, NextSlot, ttos(m_pSprite.MaxForce() / Prm.SteeringForceTweaker));
            NextSlot += SlotSize;
        }
        if (m_pSprite.ID() == 0) {
            gdi.TextAtPos(5, NextSlot, "MaxSpeed(Home/End):");
            gdi.TextAtPos(160, NextSlot, ttos(m_pSprite.MaxSpeed()));
            NextSlot += SlotSize;
        }

        //render the steering force
        if (m_pSprite.World().RenderSteeringForce()) {
            gdi.RedPen();
            Vector2D F = mul((div(m_vSteeringForce, Prm.SteeringForceTweaker)), Prm.SpriteScale);
            gdi.Line(m_pSprite.Pos(), add(m_pSprite.Pos(), F));
        }

        //render wander stuff if relevant
        if (On(behavior_type.wander) && m_pSprite.World().RenderWanderCircle()) {

            if (KEYDOWN('F')) {
                m_dWanderJitter += 1.0f * m_pSprite.getTimeElapsed();
            ...
            if (KEYDOWN('G')) {
                m_dWanderDistance += 2.0f * m_pSprite.getTimeElapsed();
            ...
            if (KEYDOWN('H')) {
                m_dWanderRadius += 2.0f * m_pSprite.getTimeElapsed();
            ...
            Vector2D m_vTCC = PointToWorldSpace(new Vector2D(m_dWanderDistance * m_pSprite.BRadius(), 0),
                    m_pSprite.Heading(),
                    m_pSprite.Side(),
                    m_pSprite.Pos());
            ...
            gdi.GreenPen();
            gdi.HollowBrush();
            gdi.Circle(m_vTCC, m_dWanderRadius * m_pSprite.BRadius());

            //draw the wander target
            gdi.RedPen();
            gdi.Circle(PointToWorldSpace(mul(add(m_vWanderTarget, new Vector2D(m_dWanderDistance, 0)), m_pSprite.BRadius()),
                    m_pSprite.Heading(),
                    m_pSprite.Side(),
                    m_pSprite.Pos()), 3);
        }
        ...
        if (m_pSprite.World().RenderDetectionBox()) {
            ...
            box.clear();
            box.add(new Vector2D(0, m_pSprite.BRadius()));
            box.add(new Vector2D(length, m_pSprite.BRadius()));
            box.add(new Vector2D(length, -m_pSprite.BRadius()));
            box.add(new Vector2D(0, -m_pSprite.BRadius()));


            if (!m_pSprite.isSmoothingOn()) {
                box = WorldTransform(box, m_pSprite.Pos(), m_pSprite.Heading(), m_pSprite.Side());
                gdi.ClosedShape(box);
            } else {
                box = WorldTransform(box, m_pSprite.Pos(), m_pSprite.SmoothedHeading(), m_pSprite.SmoothedHeading().Perp());
                gdi.ClosedShape(box);
            }
            ...
            ListIterator<BaseGameEntity> it = m_pSprite.World().Obstacles().listIterator();
            ...
            while (it.hasNext()) {
                ...
                if (curOb.IsTagged()) {
                    ...
                    if (LocalPos.x >= 0) {
                        ...
                        //than its radius + half the width of the detection box then there
                        //is a potential intersection.
                        if (Math.abs(LocalPos.y) < (curOb.BRadius() + m_pSprite.BRadius())) {
                            gdi.ThickRedPen();
                            gdi.ClosedShape(box);
                        }
                    }
        ...
        if (On(behavior_type.wall_avoidance) && m_pSprite.World().RenderFeelers()) {
            gdi.OrangePen();

            for (int flr = 0; flr < m_Feelers.size(); ++flr) {

                gdi.Line(m_pSprite.Pos(), m_Feelers.get(flr));
            }
        }

        //render path info
        if (On(behavior_type.follow_path) && m_pSprite.World().RenderPath()) {
            m_pPath.Render();
        }
        ...
        if (On(behavior_type.separation)) {
            ...
            if (KEYDOWN('S')) {
                m_dWeightSeparation += 200 * m_pSprite.getTimeElapsed();
        ...
        if (On(behavior_type.allignment)) {
            ...
            if (KEYDOWN('A')) {
                m_dWeightAlignment += 200 * m_pSprite.getTimeElapsed();
        ...
        if (On(behavior_type.cohesion)) {
            ...
            if (KEYDOWN('D')) {
                m_dWeightCohesion += 200 * m_pSprite.getTimeElapsed();
    ...
    public Vector2D Calculate() {
        ...
        //use space partitioning to calculate the neighbours of this sprite
        //if switched on. If not, use the standard tagging system
        if (!isSpacePartitioningOn()) {
            //tag neighbors if any of the following 3 group behaviors are switched on
            if (On(behavior_type.separation) || On(behavior_type.allignment) || On(behavior_type.cohesion)) {
                m_pSprite.World().TagspritesWithinViewRange(m_pSprite, m_dViewDistance);
            }
        } else {
            //calculate neighbours in cell-space if any of the following 3 group
            //behaviors are switched on
            if (On(behavior_type.separation) || On(behavior_type.allignment) || On(behavior_type.cohesion)) {
                m_pSprite.World().CellSpace().CalculateNeighbors(m_pSprite.Pos(), m_dViewDistance);
            }
        }
    ...
    public double ForwardComponent() {
        return m_pSprite.Heading().Dot(m_vSteeringForce);
    }
    ...
    public double SideComponent() {
        return m_pSprite.Side().Dot(m_vSteeringForce);
    }
    ...
    private Vector2D CalculatePrioritized() {
        ...
        if (On(behavior_type.wall_avoidance)) {
            force = mul(WallAvoidance(m_pSprite.World().Walls()),
        ...
        if (On(behavior_type.obstacle_avoidance)) {
            force = mul(ObstacleAvoidance(m_pSprite.World().Obstacles()),
        ...
        if (On(behavior_type.flee)) {
            force = mul(Flee(m_pSprite.World().Crosshair()), m_dWeightFlee);
        ...
        if (!isSpacePartitioningOn()) {
            if (On(behavior_type.separation)) {
                force = mul(Separation(m_pSprite.World().Agents()), m_dWeightSeparation);
            ...
            if (On(behavior_type.allignment)) {
                force = mul(Alignment(m_pSprite.World().Agents()), m_dWeightAlignment);
            ...
            if (On(behavior_type.cohesion)) {
                force = mul(Cohesion(m_pSprite.World().Agents()), m_dWeightCohesion);
            ...
            if (On(behavior_type.separation)) {
                force = mul(SeparationPlus(m_pSprite.World().Agents()), m_dWeightSeparation);
            ...
            if (On(behavior_type.allignment)) {
                force = mul(AlignmentPlus(m_pSprite.World().Agents()), m_dWeightAlignment);
            ...
            if (On(behavior_type.cohesion)) {
                force = mul(CohesionPlus(m_pSprite.World().Agents()), m_dWeightCohesion);
        ...
        if (On(behavior_type.seek)) {
            force = mul(Seek(m_pSprite.World().Crosshair()), m_dWeightSeek);
        ...
        if (On(behavior_type.arrive)) {
            force = mul(Arrive(m_pSprite.World().Crosshair(), m_Deceleration), m_dWeightArrive);
        ...
        if (On(behavior_type.hide)) {
            ...
            force = mul(Hide(m_pTargetAgent1, m_pSprite.World().Obstacles()), m_dWeightHide);
    ...
    private Vector2D CalculateWeightedSum() {
        if (On(behavior_type.wall_avoidance)) {
            m_vSteeringForce.add(mul(WallAvoidance(m_pSprite.World().Walls()),
                    m_dWeightWallAvoidance));
        }

        if (On(behavior_type.obstacle_avoidance)) {
            m_vSteeringForce.add(mul(ObstacleAvoidance(m_pSprite.World().Obstacles()),
                    m_dWeightObstacleAvoidance));
        }
        ...
        if (!isSpacePartitioningOn()) {
            if (On(behavior_type.separation)) {
                m_vSteeringForce.add(mul(Separation(m_pSprite.World().Agents()), m_dWeightSeparation));
            }

            if (On(behavior_type.allignment)) {
                m_vSteeringForce.add(mul(Alignment(m_pSprite.World().Agents()), m_dWeightAlignment));
            }

            if (On(behavior_type.cohesion)) {
                m_vSteeringForce.add(mul(Cohesion(m_pSprite.World().Agents()), m_dWeightCohesion));
            }
        } else {
            if (On(behavior_type.separation)) {
                m_vSteeringForce.add(mul(SeparationPlus(m_pSprite.World().Agents()), m_dWeightSeparation));
            }

            if (On(behavior_type.allignment)) {
                m_vSteeringForce.add(mul(AlignmentPlus(m_pSprite.World().Agents()), m_dWeightAlignment));
            }

            if (On(behavior_type.cohesion)) {
                m_vSteeringForce.add(mul(CohesionPlus(m_pSprite.World().Agents()), m_dWeightCohesion));
            }
        }
        ...
        if (On(behavior_type.seek)) {
            m_vSteeringForce.add(mul(Seek(m_pSprite.World().Crosshair()), m_dWeightSeek));
        }

        if (On(behavior_type.flee)) {
            m_vSteeringForce.add(mul(Flee(m_pSprite.World().Crosshair()), m_dWeightFlee));
        }

        if (On(behavior_type.arrive)) {
            m_vSteeringForce.add(mul(Arrive(m_pSprite.World().Crosshair(), m_Deceleration), m_dWeightArrive));
        }
        ...
        if (On(behavior_type.hide)) {
            ...
            m_vSteeringForce.add(mul(Hide(m_pTargetAgent1, m_pSprite.World().Obstacles()), m_dWeightHide));
        }
        ...
        m_vSteeringForce.Truncate(m_pSprite.MaxForce());
    ...
    private Vector2D CalculateDithered() {
        ...
        if (On(behavior_type.wall_avoidance) && RandFloat() < Prm.prWallAvoidance) {
            m_vSteeringForce = mul(WallAvoidance(m_pSprite.World().Walls()),
        ...
        if (!isSpacePartitioningOn()) {
            if (On(behavior_type.separation) && RandFloat() < Prm.prSeparation) {
                m_vSteeringForce.add(mul(Separation(m_pSprite.World().Agents()),
            ...
            if (On(behavior_type.separation) && RandFloat() < Prm.prSeparation) {
                m_vSteeringForce.add(mul(SeparationPlus(m_pSprite.World().Agents()),
                        m_dWeightSeparation / Prm.prSeparation));
        ...
        if (On(behavior_type.flee) && RandFloat() < Prm.prFlee) {
            m_vSteeringForce.add(mul(Flee(m_pSprite.World().Crosshair()), m_dWeightFlee / Prm.prFlee));
        ...
        if (!isSpacePartitioningOn()) {
            if (On(behavior_type.allignment) && RandFloat() < Prm.prAlignment) {
                m_vSteeringForce.add(mul(Alignment(m_pSprite.World().Agents()),
            ...
            if (On(behavior_type.cohesion) && RandFloat() < Prm.prCohesion) {
                m_vSteeringForce.add(mul(Cohesion(m_pSprite.World().Agents()),
            ...
            if (On(behavior_type.allignment) && RandFloat() < Prm.prAlignment) {
                m_vSteeringForce.add(mul(AlignmentPlus(m_pSprite.World().Agents()),
                        m_dWeightAlignment / Prm.prAlignment));
            ...
            if (On(behavior_type.cohesion) && RandFloat() < Prm.prCohesion) {
                m_vSteeringForce.add(mul(CohesionPlus(m_pSprite.World().Agents()),
                        m_dWeightCohesion / Prm.prCohesion));
        ...
        if (On(behavior_type.seek) && RandFloat() < Prm.prSeek) {
            m_vSteeringForce.add(mul(Seek(m_pSprite.World().Crosshair()), m_dWeightSeek / Prm.prSeek));
        ...
        if (On(behavior_type.arrive) && RandFloat() < Prm.prArrive) {
            m_vSteeringForce.add(mul(Arrive(m_pSprite.World().Crosshair(), m_Deceleration),
    ...
    public void SetTargetAgent1(Sprite Agent) {
        m_pTargetAgent1 = Agent;
    }

    public void SetTargetAgent2(Sprite Agent) {
    ...
    public void PursuitOn(Sprite v) {
        m_iFlags |= behavior_type.pursuit.flag();
        m_pTargetAgent1 = v;
    }

    public void EvadeOn(Sprite v) {
    ...
    public void InterposeOn(Sprite v1, Sprite v2) {
        m_iFlags |= behavior_type.interpose.flag();
        m_pTargetAgent1 = v1;
        m_pTargetAgent2 = v2;
    }

    public void HideOn(Sprite v) {
        m_iFlags |= behavior_type.hide.flag();
        m_pTargetAgent1 = v;
    }

    public void OffsetPursuitOn(Sprite v1, final Vector2D offset) {
